# llm/chains/annual_report_chain.py
...
from src.llm.models.annual_report_model import AnnualReportExtractorModel
from src.utils.env_variables import load_env_variables

class AnnualReportChain:
    def __init__(self):
        ...
        self.parser = JsonOutputParser(pydantic_object=AnnualReportExtractorModel)

        # Define the prompt template
        self.prompt = PromptTemplate(
            template="""
  ### Your job is to extract key information from the current text which is part of a company annual report, which has been taken from {title}.
  ...
  {text}
  ### Use the the following context to get a better understanding of the current text, but do not use this in the response. This is for reference only:
  {wider_context}
