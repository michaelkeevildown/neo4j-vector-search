...
from src.llm.models.policing_handbook_model import PolicingHandbookModel
from src.utils.env_variables import load_env_variables


class PolicingHandbookChain:
    def __init__(self):
        ...
        self.parser = JsonOutputParser(pydantic_object=PolicingHandbookModel)

        # Define the prompt template
        self.prompt = PromptTemplate(
            template="""
  ### Your job is to extract key information from the current text which is part of a policing handbook, which has been taken from {title}.
  ...
  {text}
  ### Use the the following context to get a better understanding of the current policing handbook, but do not use this in the response. This is for reference only:
  {wider_context}
